...
@inline(__always)
@_alwaysEmitIntoClient
public func imap<A, B, Err: Error>(
    _ xs: Array<A>,
    stream s: Stream = streamPerThread,
...
@inline(__always)
@_alwaysEmitIntoClient
public func zipWith<A, B, C, Err: Error>(
    _ xs: Array<A>,
    _ ys: Array<B>,
    stream s: Stream = streamPerThread,
    _ f: (A, B) throws(Err) -> C
) throws(Err) -> Array<C> {
...
@inline(__always)
@_alwaysEmitIntoClient
public func izipWith<A, B, C, Err: Error>(
    _ xs: Array<A>,
    _ ys: Array<B>,
    stream s: Stream = streamPerThread,
...
@inline(__always)
@_alwaysEmitIntoClient
public func permute<A, Err: Error>(
    from: Array<A>,
    into: inout Array<A>,
    combining f: (A, A) throws(Err) -> A,
